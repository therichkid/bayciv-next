<script lang="ts">
	...
	let { slug, selectedTermIds = $bindable([]), onTermIdChange = () => {}, ...restProps }: Props = $props();
...
<div class="flex w-full flex-wrap gap-2" {...restProps}>
	{#each taxonomyTerms as taxonomyTerm (taxonomyTerm.id)}
		...
		<Badge
			class="cursor-pointer bg-{color}-100 py-1 text-{color}-950 hover:bg-{color}-200"
			role="button"
